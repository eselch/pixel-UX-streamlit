...
# Question 5: Mattress Firmness
    ...
    selected_firmness_label = st.selectbox(
        ...
        index=dp.FIRMNESS_OPTIONS.index(st.session_state.answers[side_key].get("firmness_value", 2)),
        label_visibility="collapsed",
        key=f"{side_key}_firmness"
    )
    
    st.session_state.answers[side_key]["firmness_value"] = selected_firmness_label
    
    # Trigger master array update when firmness changes
    dp.update_master_array_from_firmness(side_key, selected_firmness_label)
