...
def clear_csv_data(side_key: str = None) -> None:
    """Clear CSV data from session state and reset curve to default state.
    
    Removes the CSV pressure map data and resets the master array to a flat 
    array based on current firmness.
    ...
    if side_key is None:
        ...
        for key in ["sleeper_1", "sleeper_2"]:
            if "answers" in st.session_state and key in st.session_state.answers:
                sleeper_data = st.session_state.answers[key]
                firmness = sleeper_data.get("firmness_value", 2)
                sleeper_data["num_control_points"] = 6
                array_length = dp.get_array_length()
                master_array = dp.initialize_master_array(key, firmness, array_length)
                dp.set_master_array(key, master_array)
    else:
